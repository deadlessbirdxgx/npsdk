...
class BackTestWebsocketThread(threading.Thread):

    def __init__(self, connection, thread_name, clients):
        super(BackTestWebsocketThread, self).__init__()
        self.conn = connection
        self.thread_name = thread_name
        self.clients = clients

        self.report_playindex_dict = {} # 实时行情数据 数据字典 每次取一个数据包，playindex = 0 ~ len(dict)
                                # {                  
                                #   'SH600000':playindex,    
                                #   'SH600007':playindex, 
                                #               ......
        ...
        self.klines_playindex_dict = {} # 证券K线数据 数据字典 每次取一个数据包，playindex = 0 ~ len(ktype dict)
                                # {                  
        ...
        for symbol in glb_stk_report_dict.keys():
            self.report_playindex_dict[symbol] = 0
        print(f'report coast:{time.perf_counter() - t:.8f}s')

        #初始化K线数据包回放 索引值字典
        t = time.perf_counter()
        for symbol in glb_stk_klines_dict.keys():
            if not symbol in self.klines_playindex_dict.keys():
                self.klines_playindex_dict[symbol] = {}
            
                for type in glb_stk_klines_dict[symbol].keys():
                    self.klines_playindex_dict[symbol][type] = 0
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                """
                import chardet
                input_encoding = chardet.detect(recv.encode())['encoding']
                print(input_encoding)  
                # 通过以上3条语句已经诊断 recv字符串编码 为 'utf-8'
                """

                #print('recv: ' + recv)  
                #print('服务器收到:  ' + recv)
                self.process_command_pro(recv)
                #time.sleep(1)

            except Exception as e:
                print('      >>>线程:%s 异常: %r'%(thread_name,e))
                time.sleep(2)
                #continue
                #self.conn.close()
                break
    ...
    # 处理client发来的业务命令
    def process_command_pro(self, command):
        ...
        if type == '初始化':
            ...
            for mkid in glb_marketinfo_dict.keys():
                for i in range(len(glb_marketinfo_dict[mkid])):
                    ...
                    send_all(self.clients, raw_data)
        ...
        # 股票数据?代码=SH600007&类型=实时数据&版本=20210310
            ...
            if symbol in glb_stk_report_dict.keys():
                rindex = self.report_playindex_dict[symbol]
                ...
                if rindex == len(glb_stk_report_dict[symbol]): #该代码实时行情的数据包已经回放结束
                    ...
                    send_all(self.clients, raw_data)
                    self.report_playindex_dict[symbol] = self.report_playindex_dict[symbol] + 1            
        ...
        # 股票数据?代码=SH600000&类型=1分钟线&数量=2000&版本=20210310
            ...
            if symbol in glb_stk_klines_dict.keys():
                if type in glb_stk_klines_dict[symbol].keys():
                    kindex = self.klines_playindex_dict[symbol][type]
                    ...
                    if kindex == len(glb_stk_klines_dict[symbol][type]): #该代码该K线类型的数据包已经回放结束
                        ...
                        send_all(self.clients, raw_data)
                        self.klines_playindex_dict[symbol][type] = self.klines_playindex_dict[symbol][type] + 1
        else:
            print('其它命令：', command)
            pass
...
if __name__ == '__main__':
    try:
        ...
        #self._backtest_queue_quotes = JoinableQueue() # 实时行情数据包队列
        #self._backtest_queue_klines = JoinableQueue() # K线数据包队列 
        ...
        while True:
            try:
                ...
                print('accept new client: ', thread_name)

                clients[thread_name] = connection
                count += 1
                thread = BackTestWebsocketThread(connection, thread_name, clients)
                thread.start()
